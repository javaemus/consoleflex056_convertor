...
public class .#a2600.c.1
{
	...
	***************************************************************************/
	READ_HANDLER( a2600_riot_r )
	{
		...
		/* resync */
		previous_tia_cycle = global_tia_cycle + cpu_geticount();
		riotdiff *= 3;
	...
	***************************************************************************/
	WRITE_HANDLER( a2600_TIA_w )
	{
		...
		case VSYNC:
			if (!(data & 0x02))
			{
				logerror("TIA_w - VSYNC Stop (cycles is %d)\n",cpu_geticount());
	
			}
			else if (data & 0x02)
			{
				logerror("TIA_w - VSYNC Start [Scanline=%d] [Cycle=%d] ******************\n",currentline, cpu_geticount());
				currentline = 0;
				currentframe++;
			}
			break;
		...
		case WSYNC:						/* offset 0x02 */
			if (!(data & 0x01))
			{
				...
				if( (int)((76-cpu_geticount())*3) <228)
				{
					logerror("TIA_w - WSYNC [HorizPos=%d]\n",(76-cpu_geticount())*3);
					TIA_wsync = 1;
					cpu_spinuntil_trigger(WSYNC_TRIGGER);
				}
				else
				{
					logerror("TIA_w - WSYNC Skip\n");
				}
			}
		...
		case COLUP0:						/* offset 0x06 */
			...
			logerror("TIA_w - COLUP0 Write color 0x%02x [Scanline=%d] [Cycle=%d]\n", tia.colreg.P0, currentline, cpu_geticount());
			break;
	
		case COLUP1:						/* offset 0x07 */
			tia.colreg.P1 = data;
			tia.colreg.M1 = tia.colreg.P1;	/* missile same color */
			logerror("TIA_w - COLUP1 Write color 0x%02x [Scanline=%d] [Cycle=%d]\n", tia.colreg.P1, currentline, cpu_geticount());
			break;
	
		case COLUPF:						/* offset 0x08 */
			tia.colreg.PF = data;
			tia.colreg.BL = data;			/* ball is same as playfield */
			logerror("TIA_w - COLUPF Write color 0x%02x [Scanline=%d] [Cycle=%d]\n", tia.colreg.PF, currentline, cpu_geticount());
			break;
	
		case COLUBK:						/* offset 0x09 */
			tia.colreg.BK = data;
			logerror("TIA_w - COLUBK Write color 0x%02x [Scanline=%d] [Cycle=%d]\n",tia.colreg.BK, currentline, cpu_geticount());
			break;
	...
	***************************************************************************/
	int a2600_scanline_int(void)
	{
		...
		if (TIA_wsync == 1)
		{
			logerror("WSYNC SCANLINE INT!  Resyncing CPU\n");
			cpu_trigger(WSYNC_TRIGGER);
			TIA_wsync = 0;
		}
	
		logerror("SCANLINE INT - Expected Cycles=%d, Frame #%d, Scanline #%d HorizPos=%d \n",
		...
		/* Draw the scanline using the callback */
		a2600_main_cb(0);
