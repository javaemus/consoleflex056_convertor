...
public class .#coleco.c.1
{
	...
	public static Memory_ReadAddress coleco_readmem[]={
	...
	public static Memory_WriteAddress coleco_writemem[]={
	...
	***************************************************************************/
	...
	static void coleco_nmi_interrupt (int state)
	{
	...
	static int coleco_vh_start(void)
	{
		...
		TMS9928A_int_callback(coleco_nmi_interrupt);
		return 0;
	}
	...
	***************************************************************************/
	...
	void paddle_callback (int param)
	{
	    ...
	    port5 = input_port_2_r (0);
	    if (! (port5 & 1) )
	    	JoyStat[0] = 1;//up
	    else if (! (port5 & 2) )
	    	JoyStat[0] = -1;//right
	    else
	    	JoyStat[0] = 0;//left or down
	
	    if (! (port5 & 4) )
	    	JoyStat[1] = 1;//down
	    else if (! (port5 & 8) )
	    	JoyStat[1] = -1;//left?
	    else JoyStat[1] = 0;//right or up
	
	    if (JoyStat[0] || JoyStat[1])
	    {
	        cpu_set_irq_line (0, 0, HOLD_LINE);
			printf ("port is %02x interrupt %d %d!\n", port5, JoyStat[0], JoyStat[1]);
		}
	}
	
	void init_coleco (void)
	{
		/* not sure, but this is usually the case. It's not important for IM 1 */
		cpu_irq_line_vector_w(0,0,0xff);
		timer_pulse (1.0/100.0, 0, paddle_callback);
	}
	
	/*    YEAR  NAME      PARENT    MACHINE   INPUT     INIT      COMPANY   FULLNAME */
	CONS( 1982, coleco,   0,        coleco,   coleco,   coleco,   "Coleco", "Colecovision" )
	CONS( 1982, colecoa,  coleco,   coleco,   coleco,   coleco,   "Coleco", "Colecovision (Thick Characters)" )
